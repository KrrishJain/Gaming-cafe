...
const PsSlotBooking = () => {
  ...
  const [numberOfConsoles, setNumberOfConsoles] = useState(1);
  ...
  }, [selectedSlots, numberOfConsoles]);
  ...
  const handleNumberOfConsolesChange = (event) => {
    const consoleCount = parseInt(event.target.value);
    setNumberOfConsoles(consoleCount > 3 ? 3 : consoleCount);
  };

  const calculateTotalAmount = () => {
    const amountPerSlot = 100; // Rs 100 per slot for PlayStation
    const totalSlotsSelected = selectedSlots.length;
    const totalAmount = totalSlotsSelected * numberOfConsoles * amountPerSlot;
  ...
  const handleConfirmBooking = () => {
    ...
    const newBookings = selectedSlots.map((index) => {
      ...
      return {
        ...
        name: name,
        numberOfConsoles: numberOfConsoles, // Ensure to include numberOfConsoles in each booking
      };
    });
      ...
      <div className=' px-8 py-4  bg-black text-white rounded-xl'>
        ...
        <div className='flex flex-col lg:flex-row gap-5 justify-center items-center w-full mt-8 mb-4'>
          ...
          <div className=" flex justify-center flex-col w-28">
            <label htmlFor="numberOfConsoles" className="mr-4 text-lg  text-center font-semibold">No of Consoles</label>
            <input 
              type="number" 
              id="numberOfConsoles" 
              value={numberOfConsoles} 
              onChange={handleNumberOfConsolesChange} 
              min="1" 
              max="3" 
